...
const WithdrawFulltime = ({ web3Staking, refreshDataGrid, tokenData }) => {
    ...
    const onClickApprove = async () => {
        ...
        try {
            ...
            var re = await web3Staking.methods.viewTimeUntilWithDrawFullTime(data.arg1).call();
            successMessage = `${re}`;
    ...
    return (
        <Grid container>
            <Grid item xs={12}>
                ...
                >
                    {data.loading ? <CircularProgress size={25} /> : "viewTimeUntilWithDrawFullTime"}
                </Button>
            </Grid>
            <Grid item xs={12}>
                <TextField
...
export default WithdrawFulltime