...
const ForceWithdraw = ({ web3Staking, refreshDataGrid, tokenData }) => {
    ...
    const onClickApprove = async () => {
        ...
        try {
            ...
            await web3Staking.methods.forceWithdraw().send({ from: accounts[0] });
    ...
    return (
        <Grid container>
            <Grid item xs={12}>
                ...
                >
                    {data.loading ? <CircularProgress size={25} /> : "ForceWithdraw()"}
                </Button>
            </Grid>
            <Grid item xs={12}>
                {/* <TextField
